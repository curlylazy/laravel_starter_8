...
class aOwner extends Controller
{
	public function __construct()
    {
		// page data
		$this->pesan = "";
    	$this->baseTable = "tbl_owner";
    	$this->prefix = "owner";
    	$this->pagename = "Owner";
    ...
    public function list(Request $request)
    {
		...
		$data['rows'] = DB::table($this->baseTable)
                        ->select('*')
						->orderBy('tbl_owner.kodeowner', 'desc')
						->get();
    ...
    public function acttambah(Request $request)
    {
		...
		try
		{
            ...
            $kodeowner = Csql::generateKode2("kodeowner", "OWNER", $this->baseTable);

			// simpan ke table user_login
			DB::table($this->baseTable)->insert([[
                'kodeowner' => Cfilter::FilterString($kodeowner),
                'namaowner' => Cfilter::FilterString($request->input('namaowner')),
                'emailowner' => Cfilter::FilterString($request->input('emailowner')),
                'alamatowner' => Cfilter::FilterString($request->input('alamatowner')),
                'noteleponowner' => Cfilter::FilterString($request->input('noteleponowner')),
                'dateaddowner' => Cfilter::FilterString(date("Y-m-d H:i")),
                'dateupdowner' => Cfilter::FilterString(date("Y-m-d H:i")),
			]]);
			...
			return redirect()->action([aOwner::class, 'tambah'])->with('pesaninfo', $this->pesaninfo)->with('erroract', true)->withInput();
		}

		// jika berhasil
		$this->pesaninfo = Cview::pesanSukses("Berhasil Tambah Data : <b>".$request->input('namaowner')."</b>");
		return redirect()->action([aOwner::class, 'list'])->with('pesaninfo', $this->pesaninfo);
    ...
    public function actedit(Request $request)
    {
		...
		try {
            ...
            $id = Cfilter::FilterString($request->input('kodeowner'));
			...
			return redirect()->action([aOwner::class, 'edit'], ['id' => $id])->with('pesaninfo', $this->pesaninfo)->with('erroract', true)->withInput();
		}

		// jika berhasil
		$this->pesaninfo = Cview::pesanSukses("Berhasil Update Data : <b>".$request->input('namaowner')."</b>");
		return redirect()->action([aOwner::class, 'list'])->with('pesaninfo', $this->pesaninfo);
    ...
    public function acthapus($id)
    {
        ...
        try
        {
			...
			return redirect()->action([aOwner::class, 'list'])->with('pesaninfo', $this->pesaninfo)->with('erroract', true)->withInput();
		}
